🔴 **To be Fixed**
🟡 **In Progress**
🟢 **Done**


**Tasks for Week 1** 🟢
- create database
- login
Setup XAMPP, create the 5 tables, build the Login/Session logic in PHP.
- add data for all tables 

**Tasks for Week 2**

- create the whole core of the CRUD
Build the Supplier and Product modules. Focus on clean PHP prepared statements ($stmt->prepare) for security.

**Tasks for Week 3**

- build the transaction logic 
Build the Order Entry and Tracking modules.

**Tasks for Week 4**

- Reporting & UI
Implement the Dashboard using the CTE/Subqueries. Add CSS/Tailwind for a professional look.


Data Security: Implementing more advanced role-based access for your staff.

- fix ui for my profile

