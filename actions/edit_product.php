...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        $sql = "UPDATE products SET product_code=?, product_name=?, unit_price=?, supplier_id=?, stock=? WHERE product_id=?";
...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-3xl shadow-sm border border-gray-100 mt-6">
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="mb-6">
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Current Stock Quantity</label>
                <input type="number" name="stock" value="<?= htmlspecialchars($product['stock']) ?>" required
                    class="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none transition-all font-bold text-gray-700">
            </div>
            <div>
